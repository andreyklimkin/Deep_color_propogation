...
def infer_batch(batch, refinement_net, local_transfer_net, global_transferer,
                use_only_local=False, verbose=False):
    ...
    if use_only_local:
        ...
        result_lab = refinement_output_lab
    ...
    return (refinement_output_lab, result_rgb, 
...
def load_video_frames(path, resize_size=(128, 128, 3), anti_aliasing=False):
    frames = []
...
def load_all_test_videos(video_pathes, resize_size=(128, 128, 3), anti_aliasing=False):
    ...
    for video_path in video_pathes:
        test_video_frames.append(load_video_frames(video_path, resize_size, anti_aliasing))
    return test_video_frames

def inference_test_video(frames, refinement_net, local_transfer_net, global_transferer, use_only_local=False):
    I0 = frames[0]
    I_prev = frames[0]
